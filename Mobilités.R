...
calc_bus_time_between <- function(
  ...
  # 3️⃣ Temps de marche depuis l'arrêt final
  walk_distance_end <- st_distance(arrivee_stop, arrivee)
  walk_time_end <- as.numeric(walk_distance_end) / 1000 / walk_speed_kmh * 60
  
  # Durée totale approximative
  duree_total <- walk_time_start + bus_time + walk_time_end
